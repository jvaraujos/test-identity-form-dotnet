...
namespace IdentityForm.Infrastructure.Repositories
{
    public class RepositoryAsync<T, TId> : IRepositoryAsync<T, TId> where T : AuditableEntity<TId>
    {
        ...
        public RepositoryAsync(IdentityFormContext dbContext)
        {
        ...
        {
            await _dbContext.BulkInsertOrUpdateAsync(entity.ToList());
            return;
        }
    }
    public class RepositoryNonAuditableAsync<T, TId> : IRepositoryNonAuditableAsync<T, TId> where T : NonAuditableEntity<TId>
    {
        private readonly IdentityFormContext _dbContext;

        public RepositoryNonAuditableAsync(IdentityFormContext dbContext)
        {
            _dbContext = dbContext;
        }
